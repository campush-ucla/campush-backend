...
router.get("/allnames", async function (req, res) {
    let db = client.db('campush')
    let collection = db.collection('clubs')
    let clubs = await collection.find({}, {projection: {"Organization Name": 1, "_id": 0}}).toArray()
    if (clubs !== null) {
        res.json(JSON.stringify(clubs));
    } else {
        res.json('Not found');
...
router.get("/clubs", async function (req, res) {
    ...
    let clubs = await collection.find({"Searched Category": category}, {projection: {"Organization Name": 1, "_id": 0}}).toArray()
    if (clubs !== null) {
        res.json(JSON.stringify(clubs));
    } else {
        res.json('Not found');
